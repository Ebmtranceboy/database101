...
const document = new Person({name: 'Vlad', age: 666, favoriteFoods: ['blood', 'soul', 'tears']});
  
var createAndSavePerson = function(done) {
  document.save(done);
};

//createAndSavePerson((err,data) => {if(err) console.log(err);});
...
const arrayOfPeople = [new Person({name: 'Alice', age: 1, favoriteFoods: ['milk','burrito']})
                       ,new Person({name: 'Bob', age: 2, favoriteFoods: ['soup','milk']})
                      ,new Person({name: 'Carl', age: 3, favoriteFoods: ['milk','candy','burrito']})];
const arrayOfMaries = [new Person({name: 'Mary', age: 1, favoriteFoods: ['milk']})
                       ,new Person({name: 'Mary', age: 2, favoriteFoods: ['soup','milk']})
                      ,new Person({name: 'Mary', age: 3, favoriteFoods: ['milk','candy']})];
//createManyPeople(arrayOfPeople,(err,data) => {if(err) console.log(err);});
//createManyPeople(arrayOfMaries,(err,data) => {if(err) console.log(err);});
...
var findPeopleByName = function(personName, done) {
  Person.find({name: new RegExp(personName, 'i')}, done);
};

//findPeopleByName('Carl',(err,data) => {if(err) console.log(err);else console.log(data)});

var findPeopleByFood = function(food, done) {
  Person.find({favoriteFoods: { $in:  [new RegExp(food, 'i')]}}, done);
};

//findPeopleByFood('milk',(err,data) => {if(err) console.log(err);else console.log(data)});
...
var findOneByFood = function(food, done) {
  Person.findOne({favoriteFoods: { $in:  [new RegExp(food, 'i')]}}, done);
};

//findOneByFood('soup', (err,data) => console.log(data));
...
//findPersonById('5be228fd567c7b24b14bab65',(err,data) => {if(err) console.log(err);else console.log(data)});
...
var findEditThenSave = function(personId, done) {
  ...
  Person.findById(personId, 
                  (err, data) => {
                      let foods = data.favoriteFoods.concat([foodToAdd]);
...
//findEditThenSave('5be228fd567c7b24b14bab65');
...
var findAndUpdate = function(personName, done) {
  var ageToSet = 20;
  Person.findOne({name: new RegExp(personName, 'i')}, (err,data) => {
    Person.findByIdAndUpdate(data._id, {age: ageToSet}, {new: true}, done);
  });
};

//findAndUpdate('Alice',(err,data) => {if(err) console.log(err);else console.log(data);});
...
//removeById('5be2eef247d050271e462c30', (err,data) => {if(err) console.log(err);else console.log('ok')});
...
var removeManyPeople = function(done) {
  ...
  Person.remove({name: new RegExp(nameToRemove, 'i')},done);
 };

//removeManyPeople((err,data) => {if(err) console.log(err);else console.log(data);});
...
var queryChain = function(done) {
  ...
  Person.find({favoriteFoods: { $in:  [new RegExp(foodToSearch, 'i')]}})
    .sort({name: 'asc'})
    .limit(2)
    .select('-age')
    .exec(done);
...
queryChain((err,data) => {if(err) console.log(err);else console.log(data);});
